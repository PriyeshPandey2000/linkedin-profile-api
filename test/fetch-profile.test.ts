// Tests for the pure, network-free pieces of lib/fetch-profile.ts.
...
import { parsePaginatedPage, extractLanguages, parseTopCard } from '../lib/fetch-profile';

function rawCertPage(entryCount: number): string[] {
...
test('parsePaginatedPage: a genuinely FULL hr-divided page does not stop pagination', () => {
  ...
  assert.ok(result.items[0].__rawPage, 'hr-divided pages are tagged for lib/parsers.ts to group');
});
...
function rawSkillsPage(itemCount: number): string[] {
...
test('parseTopCard: picks the highest-resolution photo from the preload imageSrcSet hint', () => {
  ...
  `;
  const result = parseTopCard(html);
  assert.ok(result.profileImage, 'profileImage should be set');
