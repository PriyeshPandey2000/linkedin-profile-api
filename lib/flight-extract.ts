...
// flight-resolver.ts) into clean structured section data. Same spirit as
...
export function isNoise(s: string): boolean {
  ...
  // fetch-profile.ts's skills grouping)
...
export function cleanStrings(strings: string[]): string[] {
  const out: string[] = [];
...
interface SectionNode {
  ...
  __import?: true;
  [k: string]: ResolvedNode | undefined;
}

function findSections(resolved: Record<string, ResolvedNode>): Record<string, SectionNode> {
  ...
  function walk(node: ResolvedNode): void {
    ...
    if (node && typeof node === 'object' && !(node as { __import?: true }).__import) {
      ...
      visited.add(node);
      const asSection = node as unknown as SectionNode;
      if (typeof asSection.observabilityIdentifier === 'string') {
        const name = asSection.observabilityIdentifier.split('.').pop();
        if (name && !(name in sections)) sections[name] = asSection;
      }
      for (const v of Object.values(node)) walk(v as ResolvedNode);
    }
  }
...
export function extractSectionsFromResponse(rawText: string): SectionExtraction {
  ...
  for (const [name, node] of Object.entries(sectionMap)) {
    const scope: ResolvedNode = node.children !== undefined ? node.children : (node as unknown as ResolvedNode);
...
export function groupByDivider(rawLines: string[], dividerToken = 'hr'): string[][] {
  const groups: string[][] = [];
  let current: string[] = [];
