...
async function fetchComponent(slug, componentId) {
...
async function fetchDetailsPage(slug, sectionName) {
...
async function fetchPaginated(slug, sectionName) {
  ...
  for (let start = 0; start < 300; start += count) {
    ...
    const res = await fetch(entry.url, { method: 'POST', headers: pageHeaders, body: JSON.stringify(body) });
    ...
    if (rawPageLines.length === 0) break;
    ...
    if (hasEndorseMarker) {
      ...
      for (const line of clean) {
        if (line === 'Endorse' || line.startsWith('Endorse ')) {
          if (chunk.length) {
            allLines.push({ name: chunk[0], context: chunk.slice(1) });
            itemCount += 1;
            chunk = [];
          }
        } else {
      ...
      stop = itemCount < count;
    } else {
      const hrCount = rawPageLines.filter((l) => l === 'hr').length;
      if (hrCount > 0) {
        // Education/Certifications: "hr" delimits each item (verified: a
        // page of 10 certifications produces exactly 10 "hr" markers).
        // Keep the RAW lines (not cleaned) so lib/parsers.js can group by
        // that divider downstream -- tagged so the caller can tell this
        // apart from the {name,context} skills shape.
        allLines.push({ __rawPage: rawPageLines });
        stop = hrCount < count;
      } else {
        // Self-view (own profile, edit-mode UI) has NEITHER an "Endorse"
        // marker NOR an "hr" divider on Skills pages -- flat list of
        // names with no clean per-item boundary at all (documented,
        // accepted limitation, see README -- self-view isn't the
        // product's primary use case). Fall back to one item per cleaned
        // line rather than losing the data entirely.
        clean.forEach((line) => allLines.push({ name: line, context: [] }));
        stop = clean.length < 8;
      }
    }
    if (stop) break;
...
async function fetchTopCard(slug) {
  const entry = templates.componentTemplate;
  const headers = stripHopHeaders(entry.headers);
...
async function fetchProfile(profileUrl) {
  ...
  const topCard = await fetchTopCard(slug);

  const [aboveActivityRes, experienceRes, part1Res, skillsPreviewRes, languagesRes] = await Promise.all([
    fetchComponent(slug, COMPONENT_IDS.aboveActivity),
    fetchComponent(slug, COMPONENT_IDS.experience),
    fetchComponent(slug, COMPONENT_IDS.part1),
    fetchComponent(slug, COMPONENT_IDS.skills),
    fetchComponent(slug, COMPONENT_IDS.languages),
  ]);
  ...
  let experienceRaw;
  const expDetails = await fetchDetailsPage(slug, 'experience');
  ...
  const [fullSkills, fullEducationPages, fullCertificationPages] = await Promise.all([
    needsFullSkills ? fetchPaginated(slug, 'skills') : Promise.resolve(null),
    needsFullEducation ? fetchPaginated(slug, 'education') : Promise.resolve(null),
    needsFullCerts ? fetchPaginated(slug, 'certifications') : Promise.resolve(null),
  ]);
