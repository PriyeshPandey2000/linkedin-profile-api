// Run: node dist/lib/fetch-profile.js <profile-url>
...
import { parseFlightStream, resolveAll, extractStrings } from './flight-resolver';
import type { ResolvedNode } from './flight-resolver';
import { extractSectionsFromResponse, cleanStrings } from './flight-extract';
import {
...
import type { ExperienceEntry, EducationEntry, CertificationEntry, SkillEntry } from './parsers';

export type Signal = AbortSignal;

// __dirname-relative, not CWD-relative -- works regardless of where the
// process is launched from (matters once this is required by src/server.ts
...
interface TemplatesShape {
  componentTemplate: FlightTemplate;
  detailsPageTemplates: Record<string, FlightTemplate>;
  paginationTemplates: Record<string, FlightTemplate>;
}

function loadTemplates(): TemplatesShape {
  if (!fs.existsSync(TEMPLATES_PATH)) {
    throw new Error(`${TEMPLATES_PATH} not found (see README setup instructions).`);
  }
  return JSON.parse(fs.readFileSync(TEMPLATES_PATH, 'utf8')) as TemplatesShape;
}
...
// string), not two.
function deriveCsrfToken(cookieString: string): string | null {
...
async function fetchComponent(
  ...
  const originalSlug = JSON.parse(entry.postData).clientArguments.payload.vanityName as string;
...
export function parsePaginatedPage(rawPageLines: string[], count: number): PaginatedPageResult {
  ...
  if (hrCount > 0) {
    ...
    // cleaned) so lib/parsers.ts can group by that divider downstream --
...
async function fetchPaginated(
  ...
  if (!entry) return [];
  const headers = stripHopHeaders(entry.headers);
  const bodyTemplate = JSON.parse(entry.postData) as {
    clientArguments: { payload: { vanityName: string; start: number; count: number } };
  };
  const originalSlug = bodyTemplate.clientArguments.payload.vanityName;

  const allLines: PaginatedItem[] = [];
  const count = 10;
  for (let start = 0; start < 300; start += count) {
    const body = JSON.parse(JSON.stringify(bodyTemplate)) as typeof bodyTemplate;
    ...
    const rawPageLines = extractStrings(resolved['0'] as ResolvedNode); // NOT cleaned yet -- "hr"
...
function libError(code: string, message: string): Error & { code?: string } {
...
export function parseTopCard(html: string): TopCardResult {
  const $ = cheerio.load(html);
  const junkHeadings = /notification|jump menu/i;
  const headings: string[] = [];
  $('h1, h2').each((_: number, el) => {
    const text = $(el).text().trim();
    if (text && text.length < 80 && !junkHeadings.test(text)) headings.push(text);
  });
  const name = headings[0] || null;
  const result: TopCardResult = { name, headline: null, location: null, profileImage: null };
...
// (CLI, tests) without one. When the caller (profileService.ts) passes
...
// profileService.ts's comment on why that distinction matters).
export async function fetchProfile(
  profileUrl: string,
  { signal }: { signal?: Signal } = {}
): Promise<ProfileDataShape> {
  ...
  let experienceRaw: string[] | undefined;
  ...
  return {
    ...
    skills: fullSkills
      ? (fullSkills[0] && fullSkills[0].__rawPage
          ...
          : (fullSkills as unknown as SkillEntry[]))
      : (skillsPreview as unknown as SkillEntry[]),
    languages,
    projects,
  } as ProfileDataShape;
}
...
function flattenRawPages(taggedPages: PaginatedItem[]): string[] {
  const lines: string[] = [];
...
export function extractLanguages(rawLanguagesText: string): { name: string; proficiency: string }[] {
...
// bio text.
export function extractAbout(rawAboveActivityText: string): string | null {
  ...
  // Quirk faithfully preserved from the original: when no "About" heading
  // is present, the original returned the raw cleaned line ARRAY (not a
  // joined string) -- rare edge case, kept intact so behavior is unchanged.
  if (aboutIdx === -1) return cleaned.length ? (cleaned as unknown as string) : null;
...
declare const require: NodeRequire & { main?: NodeModule };

if (require.main === module) {
  // Only load .env when run directly as a CLI (e.g. for manual testing).
  // When required as a module (src/services/profileService.ts), the
  // caller (server.ts / api/index.ts) is responsible for loading env --
